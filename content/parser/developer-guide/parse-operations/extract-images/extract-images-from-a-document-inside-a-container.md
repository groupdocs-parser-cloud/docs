...
# Set the environment variables CLIENT_ID, CLIENT_SECRET and JWT_TOKEN before running these commands.
...
# First get JSON Web Token
# Please get your Client Id and Client Secret from https://dashboard.groupdocs.cloud/applications.
# Set the environment variables $env:CLIENT_ID, $env:CLIENT_SECRET and $env:JWT_TOKEN before running these commands.
...
curl.exe -v "https://api.groupdocs.cloud/v1.0/parser/text" `
  ...
  -d "{ 'StartPageNumber': 1, 'CountPagesToExtract': 2, 'FileInfo': { 'FilePath': 'pdf\\PDF with attachements.pdf', 'StorageName': '', 'Password': 'password' }, 'ContainerItemInfo': { 'RelativePath': 'template-document.pdf' } }"
```
...
REM Set the environment variables CLIENT_ID, CLIENT_SECRET and JWT_TOKEN before running these commands.
curl -v "https://api.groupdocs.cloud/connect/token" ^
  -X POST ^
  -d "grant_type=client_credentials&client_id=%CLIENT_ID%&client_secret=%CLIENT_SECRET%" ^
  -H "Content-Type: application/x-www-form-urlencoded" ^
